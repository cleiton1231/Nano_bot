...
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `planning`                        | Obrigatório antes de qualquer mudança complexa multi-arquivo, multi-etapa, ou que toque schema/lógica core (ex: RAG, banco vetorial, adicionar dependência). Une esclarecimento prévio, rigor técnico, fundamentação em código real e crítica independente por subagente. Substitui as antigas `concise-planning` e `grounded-planning`. |
| `independent-review`              | Obrigatório após qualquer implementação não-trivial, antes de declarar a tarefa como concluída/pronta. Dispara revisão crítica por subagente para auditar complexidade, duplicação e tratamento de erros. |
| `test-driven-development`         | Obrigatório quando implementar lógica ou módulos novos (ex: pipeline RAG, parsers, stores). Seguir estritamente o ciclo Red-Green-Refactor com output literal do teste falhando antes da implementação. |
| `dependency-audit`                | Obrigatório antes de qualquer dependência Python nova (`pyproject.toml`) ou modelo GGUF novo. Aplica-se também a **conversões de modelo** — a cautela do reranker quebrado (Seção 4) é o motivo desta linha existir: nunca aceitar um GGUF comunitário sem teste de sanidade real.                                                            |
...
| `top-web-vulnerabilities`         | Checklist OWASP complementar ao security-audit para validação de injeções (SQL/comando), SSRF, controle de acesso e segredos em texto puro.                                                                                                                |
| `local-llm-serving`               | Usar ao dimensionar VRAM, configurar portas e flags de inferência local (`llama-server`), e testar endpoints de embedding, reranker e geração.                                                                                                                                                                            |
