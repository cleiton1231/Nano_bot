...
| `security.rateLimit.requestsPerMinute` | `60` | Requests per minute allowed per remote address on the OpenAI-compatible API server. `0` disables the limiter. This is a fixed window held in the gateway process, so a client can burst up to 2x across a window boundary and the counter resets on restart; behind a reverse proxy every request shares the proxy's address. Use your proxy's limiter for public exposure. |
| `security.auditLog` | `true` | Write structured security events to `~/.nanobot/security.log` as JSON Lines: auth failures, rate-limit denials, SSRF blocks, workspace-boundary violations, `exec` denials, and session-retention sweeps. Event metadata only — never message or command content. The file is created `0600` and is not rotated. |
| `security.sessionMaxAgeDays` | `0` | Delete conversation sessions untouched for this many days. The sweep runs once at gateway startup, so a long-running gateway does not prune until it restarts. `0` disables retention and keeps history indefinitely. Deletion is permanent and unprompted. |

**File permissions**: nanobot creates `~/.nanobot` and its runtime subdirectories with mode `0700`, and `config.json`, `security.log`, `pairing.json`, and the OAuth token stores under `auth/` with mode `0600`. Existing files and directories found looser are tightened in place on the next write. On Windows these modes are not enforced by the filesystem.
